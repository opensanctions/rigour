...
//
// `rust/data/names/persons.txt` is committed as plain UTF-8 (~8.5 MB)
...
// and the Rust code in `names::persons` picks it up via
// `include_bytes!(concat!(env!("OUT_DIR"), "/persons.txt.zst"))`.
...
// a cargo warning. Callers of `names::persons::raw()` will see the
...
fn main() {
    ...
    let source_path = PathBuf::from("data/names/persons.txt");
    let dest_path = out_dir.join("persons.txt.zst");

    println!("cargo:rerun-if-changed=data/names/persons.txt");
    println!("cargo:rerun-if-changed=build.rs");

    let Ok(bytes) = fs::read(&source_path) else {
        println!(
            "cargo:warning=rust/data/names/persons.txt not found — \
    ...
    // the full corpus; not a build-time hot spot. Mirrors the
    // `write_zstd` helper's level in `genscripts/util.py`.
    let compressed = zstd::encode_all(bytes.as_slice(), 19).expect("zstd compress persons.txt");
    fs::write(&dest_path, &compressed).expect("write compressed persons.txt.zst");
}
