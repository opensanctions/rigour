# This file is automatically generated, do not edit it.
from typing import Tuple, Dict

PERSON_NAME_PREFIXES: Tuple[str, ...] = (
    "Admiral",
    "Amb",
    "Ambassador",
    "Associate Professor",
    "Baron",
    "Baroness",
    "Baronet",
    "Baronetess",
    "Brigadier",
    "Capt.",
    "Captain",
    "Chair",
    "Chairman",
    "Chairwoman",
    "Col.",
    "Colonel",
    "Commodore",
    "Company Secretary",
    "Count",
    "Countess",
    "Dame",
    "Director",
    "Doctor",
    "Dott",
    "Dott.sr",
    "Dott.sr",
    "Dott.sra",
    "Dott.sra",
    "Dott.ssa",
    "Dott.ssa",
    "Dott.ssa",
    "Dottor",
    "Dottore",
    "Dr",
    "Duchess",
    "Emir",
    "Emira",
    "Emperor",
    "Empress",
    "Ex-Chair",
    "Ex-Chairman",
    "Ex-Chairwoman",
    "Ex-Company Secretary",
    "Ex-Director",
    "Ex-Managing Director",
    "Excellency",
    "Field Marshal",
    "Fr",
    "Frau",
    "Fräulein",
    "General",
    "Grand Duchess",
    "Grand Duke",
    "HRH",
    "Her Excellency",
    "Her Excellency",
    "Her Grace",
    "Her Higness",
    "Her Honor",
    "Her Honour",
    "Her Majesty",
    "Her Royal Higness",
    "Herr",
    "His Excellency",
    "His Excellency",
    "His Grace",
    "His Higness",
    "His Honor",
    "His Honour",
    "His Majesty",
    "His Royal Higness",
    "Hon",
    "Honorable",
    "Honourable",
    "Hr",
    "King",
    "Lady",
    "Laird",
    "Lieutenant Colonel",
    "Lieutenant General",
    "Lord",
    "Madam",
    "Madame",
    "Maj.",
    "Major",
    "Managing Director",
    "Marchioness",
    "Marquess",
    "Master",
    "Miss",
    "Misses",
    "Missis",
    "Missus",
    "Mister",
    "Mistress",
    "Mixter",
    "Mme",
    "Mmme",
    "Monsieur",
    "Mr",
    "Mrs",
    "Ms",
    "Mx",
    "President",
    "Prince",
    "Princess",
    "Private",
    "Prof",
    "Prof. Dr",
    "Professor",
    "Professor Dr",
    "Präsident",
    "Pvt.",
    "Queen",
    "Rear Admiral",
    "Rear-Admiral",
    "Reverend",
    "Right Honorable",
    "Right Honourable",
    "Rt Hon",
    "Rt. Hon.",
    "Seigneur",
    "Senor",
    "Senorita",
    "Shaikh",
    "Sheik",
    "Sheikh",
    "Sir",
    "Sr",
    "Sultan",
    "Sultana",
    "The",
    "The Hon",
    "The Honble",
    "The Honorable",
    "The Honorable Lady",
    "The Honourable",
    "The Honourable Lady",
    "The Most Hon",
    "The Most Hon.",
    "The Most Honble",
    "The Most Honorable",
    "The Most Honourable",
    "The Most Noble",
    "The Much Hon",
    "The Much Hon.",
    "The Much Honored",
    "The Much Honoured",
    "The Prince",
    "The Princess",
    "The Princess Royal",
    "The Reverend",
    "The Right Honorable",
    "The Right Honourable",
    "The Worshipful",
    "Worshipful",
)

ORG_NAME_PREFIXES: Tuple[str, ...] = ("La", "Le", "The")

STOPWORDS: Tuple[str, ...] = (
    "&",
    "a",
    "a",
    "al",
    "al",
    "als",
    "an",
    "an",
    "and",
    "as",
    "at",
    "auf",
    "avec",
    "bei",
    "by",
    "da",
    "das",
    "de",
    "de",
    "de",
    "degli",
    "del",
    "della",
    "delle",
    "der",
    "des",
    "di",
    "die",
    "do",
    "du",
    "el",
    "em",
    "en",
    "en",
    "es",
    "et",
    "et",
    "for",
    "from",
    "für",
    "in",
    "in",
    "into",
    "ja",
    "la",
    "la",
    "las",
    "le",
    "le",
    "los",
    "mit",
    "of",
    "on",
    "or",
    "par",
    "para",
    "per",
    "por",
    "por",
    "pour",
    "the",
    "to",
    "ul",
    "un",
    "un",
    "una",
    "unas",
    "und",
    "une",
    "unos",
    "ve",
    "von",
    "voor",
    "wa",
    "with",
    "y",
    "y",
    "za",
    "zu",
    "à",
    "és",
    "а",
    "а",
    "але",
    "без",
    "в",
    "в",
    "все",
    "від",
    "для",
    "для",
    "до",
    "з",
    "з",
    "за",
    "за",
    "и",
    "из",
    "к",
    "как",
    "на",
    "на",
    "но",
    "о",
    "от",
    "по",
    "по",
    "при",
    "при",
    "про",
    "с",
    "це",
    "что",
    "що",
    "это",
    "як",
    "і",
    "أو",
    "أي",
    "إلى",
    "ال",
    "ذلك",
    "على",
    "عن",
    "في",
    "في",
    "كل",
    "مع",
    "من",
    "هذا",
    "و",
    "で",
    "と",
    "に",
    "の",
    "は",
    "一个",
    "一些",
    "为",
    "和",
    "在",
    "是",
    "有",
    "的",
    "这",
    "那",
)

STOPPHRASES: Tuple[str, ...] = (
    "A.K.A",
    "A/K/A",
    "F/K/A",
    "a.k.a",
    "a.k.a.",
    "a/k/a",
    "also known as",
    "dba",
    "doing business as",
    "earlier known as",
    "f/k/a",
    "former",
    "formerly",
    "formerly known as",
    "presently known as",
    "previously known as",
    "trading as",
)

ORG_SYMBOLS: Dict[str, Tuple[str, ...]] = {
    "CO": (
        "Etaireia",
        "co",
        "companhia",
        "compania",
        "companie",
        "company",
        "compañía",
        "corporation",
        "firm",
        "firma",
        "kampaniia",
        "kampaniya",
        "komaniya",
        "kompani",
        "kompania",
        "kompanii",
        "kompaniia",
        "kompaniy",
        "kompaniya",
        "kompānija",
        "întreprinderea",
        "Εταιρεία",
        "компана",
        "компани",
        "компаний",
        "компания",
        "компанія",
        "коспания",
        "къмпани",
        "сompany",
        "شركة",
        "ファーム",
        "企业",
        "企業",
        "公司",
    ),
    "ORG": (
        "Organismos",
        "org",
        "organisaatio",
        "organisasjon",
        "organisasjonen",
        "organisatie",
        "organisation",
        "organisations",
        "organizace",
        "organizaci",
        "organizacija",
        "organizacije",
        "organizaciju",
        "organizacion",
        "organización",
        "organizacja",
        "organizatia",
        "organization",
        "organizatsii",
        "organizatsiya",
        "organizația",
        "organizácia",
        "organizácie",
        "organzation",
        "orhanizatsiia",
        "orhanizatsiino",
        "orhanizatsiya",
        "Οργανισμός",
        "организации",
        "организация",
        "организациями",
        "организація",
        "органищация",
        "ファーム",
        "協会",
        "団体",
    ),
    "CORP": (
        "corp",
        "corporaciia",
        "corporacion",
        "corporación",
        "corporate",
        "corporation",
        "corporations",
        "corporaton",
        "corporația",
        "koroporatsiya",
        "korporacija",
        "korporatsiia",
        "korporatsiya",
        "korporatsiyi",
        "sociedade",
        "societatea",
        "società",
        "Εταιρικός Όμιλος",
        "корпарация",
        "корпорации",
        "корпораций",
        "корпорация",
        "корпорація",
        "корпорації",
        "сorporation",
        "مؤسسة",
        "協力的な",
        "合作社",
        "生活協同組合",
    ),
    "COOP": (
        "Synetairismos",
        "co-op",
        "co-op society",
        "co-operation",
        "co-operative",
        "co-operative society",
        "coop",
        "coop society",
        "cooperatieve",
        "cooperation",
        "cooperativa",
        "cooperative",
        "cooperative society",
        "cooperatives",
        "cooperativo",
        "cooperativă",
        "coopératif",
        "coopération",
        "coopérative",
        "corporativo",
        "corporativos",
        "kooperatif",
        "kooperativa",
        "kooperatsia",
        "kooperattiva",
        "koperasi",
        "sociedad cooperativa",
        "spółdzielni",
        "Συνεταιρισμός",
        "кооперация",
    ),
    "ASSOC": ("association", "associations", "ассоциация", "объединение"),
    "PARTNER": (
        "partner",
        "partners",
        "партнер",
        "партнери",
        "партнеры",
        "партнёр",
        "спільник",
        "спільники",
        "الشركاء",
        "伙伴",
        "共同経営者",
        "合伙人",
        "合夥人",
        "組合員",
    ),
    "CENTER": (
        "Centro",
        "Centrum",
        "center",
        "centr",
        "centras",
        "centre",
        "centro",
        "cntr",
        "tsentr",
        "tsentrov",
        "центр",
        "центров",
        "مركز",
    ),
    "CULTURE": ("culture", "kultura", "культуры"),
    "HOLDING": (
        "Symmetoches",
        "Symmetochon",
        "holding",
        "holdings",
        "kholding",
        "kholdyng",
        "xoлдиhг",
        "Συμμετοχές",
        "Συμμετοχών",
        "проведение",
        "хлдинг",
        "холдинг",
        "القابضة",
        "保持",
        "保有資産",
        "集体",
    ),
    "BUSINESS": (
        "business",
        "businesses",
        "Επιχείρηση",
        "Μπίζνες",
        "бизнес",
        "бізнес",
        "деловая",
        "деловой",
        "أعمال",
        "商",
        "商业",
    ),
    "NOMINEE": ("nominee", "nominees"),
    "BRANCH": ("branch", "branch of", "filiale", "zweigstelle", "филиал"),
    "CONCERN": (
        "concern",
        "consorcio",
        "consórcio",
        "group of companies",
        "kontsern",
        "konzern",
        "концерн",
        "مجموعة",
    ),
    "CONSORTIUM": (
        "consorcio",
        "consortile",
        "consortium",
        "consortum",
        "consórcio",
        "konsortsium",
    ),
    "SERVICE": (
        "service",
        "services",
        "servicio",
        "servicios",
        "servico",
        "servicos",
        "servizi",
        "servizio",
        "serviço",
        "serviços",
        "обслуживания",
        "сервис",
        "служба",
        "службы",
    ),
    "DEFENCE": (
        "Verteidigung",
        "defence",
        "defensa",
        "defense",
        "defensie",
        "defensiv",
        "defesa",
        "difesa",
        "défense",
        "försvar",
        "protecția",
        "verdediging",
        "оборон",
        "оборонный",
    ),
    "DEPT": (
        "Abteilung",
        "afdeling",
        "afdelingen",
        "departament",
        "departamento",
        "departamentul",
        "department",
        "dept.",
        "dipartimento",
        "département",
        "отдел",
    ),
    "SPECIAL": (
        "Speciaal",
        "Specijalni",
        "Specjalnych",
        "special",
        "speciale",
        "specială",
        "Специальный",
    ),
    "SPV": ("special purpose vehicle", "spv", "vehículo de propósito especial"),
    "TECH": (
        "technalohiy",
        "technical",
        "technik",
        "technologia",
        "technological",
        "technologickou",
        "technologická",
        "technologie",
        "technologies",
        "technologii",
        "technologijų",
        "technologique",
        "technologií",
        "technology",
        "technológiai",
        "technológiák",
        "technológií",
        "tecnologia",
        "tecnologias",
        "tecnologies",
        "tecnología",
        "tehnologia",
        "tehnologică",
        "tehnologie",
        "tehnologiei",
        "tehnologii",
        "tehnologiju",
        "tehnološki",
        "tehnološko",
        "tekhnichnyi",
        "tekhnologi",
        "tekhnologii",
        "tekhnologiy",
        "tekhnologiya",
        "tekhnology",
        "tekhnolohii",
        "tekhnolohiia",
        "tekhnolohiyi",
        "teknologii",
        "teknologiya",
        "teknoloġija",
        "τεχνολογία",
        "τεχνολογίας",
        "τεχνολογίες",
        "техника",
        "технический",
        "технического",
        "технической",
        "технологии",
        "технологий",
        "технологическая",
        "технологический",
        "технология",
        "технологиями",
        "технологій",
        "технологічне",
        "технологія",
        "технології",
        "технолоджи",
        "технолоджі",
        "технікал",
        "тэхналогій",
        "тэхналогіі",
        "тэхники",
        "тэхніка",
        "工艺学",
        "技术",
    ),
    "INSURANCE": (
        "insurance",
        "insurance company",
        "insurans",
        "insurer",
        "versicherung",
        "versicherungen",
        "versicherungsgesellschaft",
        "verzekeringsmaatschappij",
        "verzekeringsmaatschappijen",
        "страховая",
        "страхового",
    ),
    "INTEL": (
        "intel",
        "inteligencia",
        "inteligjenca",
        "inteligjencës",
        "inteligência",
        "intelijen",
        "intelligence",
        "wywiadu",
        "разведывательное",
    ),
    "PROPERTY": (
        "properties",
        "property",
        "собственности",
        "собственность",
        "собственностью",
    ),
    "REALTY": (
        "immobilien",
        "immobilier",
        "imobiliare",
        "imobiliária",
        "inmobiliaria",
        "real estate",
        "realtor",
        "realty",
        "недвижимость",
        "недвижимостью",
        "недвижимостями",
        "недвижимостях",
    ),
    "HOUSE": (
        "casa",
        "dom",
        "haus",
        "house",
        "housing",
        "huis",
        "hus",
        "maison",
        "дом",
    ),
    "ROYAL": ("kunglig", "königlich", "royal", "royale"),
    "SUPPLY": ("supplier", "supplies", "supply"),
    "EQUIPMENT": (
        "equipement",
        "equipment",
        "оборудованиe",
        "оборудование",
        "оборудованию",
        "обрудование",
        "обрудовании",
        "обрудования",
        "обрудованиям",
        "обрудованиями",
        "обрудованиях",
    ),
    "INNOVATION": (
        "innovation",
        "innovations",
        "innovatsiya",
        "инновационная",
        "инновационное",
    ),
    "AVIATION": (
        "aerospace",
        "aviación",
        "aviation",
        "luftfahrt",
        "авиационная",
        "авиационные",
        "авиационный",
        "авиация",
    ),
    "AIRCRAFT": ("aircraft", "airplane", "flugzeug", "аэроплан", "самолет", "самолеты"),
    "AIRLINE": (
        "airline",
        "airlines",
        "авиакомпании",
        "авиакомпаний",
        "авиакомпанию",
        "авиакомпания",
        "авиаперевозчик",
        "авиаперевозчика",
        "авиаперевозчикам",
        "авиаперевозчиках",
        "авиаперевозчики",
    ),
    "AIRPORT": (
        "aerodrom",
        "aerodrome",
        "aerodromes",
        "aerodromi",
        "aerodromo",
        "aeroporti",
        "aeroporto",
        "aeroportos",
        "aeropuerto",
        "aeropuertos",
        "aeródromo",
        "aeródromos",
        "airport",
        "airports",
        "aérodrome",
        "aérodromes",
        "aéroport",
        "aéroports",
        "flughafen",
        "flughäfen",
        "havaalanı",
        "letištní",
        "letiští",
        "letiště",
        "lotniska",
        "lotnisko",
        "luchthaven",
        "luchthavens",
        "lufthaven",
        "port lotniczy",
        "авиапорт",
        "аеродром",
        "аеродроми",
        "аеропорт",
        "аеропорти",
        "аэропорт",
        "аэропорта",
        "аэропортов",
        "аэропорты",
        "летное поле",
        "летовище",
        "エアポート",
        "机场",
        "機場",
        "空港",
        "航空港",
        "飛機場",
        "飛行場",
        "飞机场",
        "공항",
        "비행장",
        "항공항",
    ),
    "FLIGHT": (
        "flight",
        "flights",
        "полет",
        "полетам",
        "полетами",
        "полетах",
        "полете",
        "полетов",
        "полеты",
        "рейс",
        "рейсов",
        "рейсы",
    ),
    "MARITIME": (
        "marine",
        "maritim",
        "maritime",
        "морская",
        "морские",
        "морских",
        "морского",
        "морской",
    ),
    "NAVAL": ("naval", "navy"),
    "RAILWAY": (
        "railway",
        "railway company",
        "railways",
        "железнодорожной",
        "железнодорожный",
        "железной",
    ),
    "AUTO": (
        "auto",
        "automobile",
        "automobilei",
        "automobilele",
        "automobilelor",
        "automobiles",
        "automobiliai",
        "automobilii",
        "automobilis",
        "automobilov",
        "automobilu",
        "automobily",
        "automobilya",
        "automobilyi",
        "automotive",
        "automóviles",
        "car",
        "vehicle",
        "vehicular",
        "автомобиль",
    ),
    "NUCLEAR": (
        "niwclear",
        "nucleair",
        "nuclear",
        "nucleare",
        "nucleares",
        "nucléaire",
        "nucléaires",
        "nuklear",
        "nuklearna",
        "nuklearne",
        "nuklearny",
        "nyuklia",
        "nükleer",
        "πυρηνική",
        "πυρηνικό",
        "πυρηνικός",
        "ядерна",
        "ядерная",
        "ядерне",
        "ядерний",
        "ядерное",
        "ядерные",
        "ядерный",
        "ядерні",
        "核的",
        "核能",
    ),
    "OIL": (
        "neft",
        "oil",
        "petrochemical",
        "petroleum",
        "нефтепереработка",
        "нефть",
        "нефтяная",
        "нефтяной",
        "татнефть",
    ),
    "STEEL": (
        "metal",
        "metallurgical",
        "metallurgy",
        "steel",
        "steelworks",
        "металлургический",
        "сталь",
    ),
    "MINING": ("bergbau", "mines", "mining", "горнодобывающая"),
    "FACTORY": (
        "fabbrica",
        "fabbriche",
        "fabriek",
        "fabrik",
        "fabrika",
        "fabrique",
        "factories",
        "factory",
        "plant",
        "rasteniye",
        "usine",
        "zavod",
        "závod",
        "завод",
        "растение",
        "фабрика",
        "厂",
        "厂子",
        "厂家",
        "工厂",
    ),
    "FUND": (
        "fond",
        "fondo",
        "fondovy",
        "fonds",
        "fund",
        "fundo",
        "funds",
        "фонд",
        "صندوق",
    ),
    "INTL": (
        "int'l",
        "internacionais",
        "internacional",
        "internacionales",
        "internantional",
        "internasional",
        "international",
        "internationale",
        "internationales",
        "internatsionale",
        "internazionale",
        "internazionali",
        "internațional",
        "intl",
        "интернациональная",
        "интернациональные",
        "интернациональный",
        "інтернейшнл",
        "国际",
        "国际的",
        "国際",
        "国際的",
    ),
    "GRP": (
        "Csoport",
        "Grupa",
        "Grupul",
        "Grúpa",
        "Skupina",
        "group",
        "groupe",
        "groupement",
        "grup",
        "grupo",
        "grupp",
        "gruppo",
        "grupė",
        "hrupa",
        "Όμιλος",
        "Ομάδα",
        "груп",
        "групп",
        "группа",
        "группы",
        "グループ",
        "团",
        "团体",
        "集体",
    ),
    "MGMT": (
        "Epicheirisi",
        "gestión",
        "management",
        "Διαχείριση",
        "мanagement",
        "менеджмент",
        "менеджмента",
        "менеджменту",
        "упарвлению",
        "упарвления",
        "управлениe",
        "управление",
        "управлению",
        "управления",
        "упрвлению",
        "ўпраўлення",
        "إدارة",
        "机场",
        "管理",
        "辖",
    ),
    "FED": (
        "federal",
        "federalna",
        "federalny",
        "federalnyy",
        "federatsiyi",
        "federatsyy",
        "federálny",
        "federální",
        "federāla",
        "глсударственное",
        "фeдеральное",
        "фдеральная",
        "федеpальное",
        "федеарльная",
        "федеоальная",
        "федералльная",
        "федералная",
        "федералное",
        "федеральна",
        "федеральная",
        "федеральне",
        "федеральний",
        "федеральних",
        "федеральноая",
        "федерального",
        "федеральное",
        "федеральной",
        "федеральном",
        "федеральному",
        "федеральної",
        "федеральные",
        "федеральный",
        "федеральным",
        "федеральными",
        "федеральных",
        "федерации",
        "федерельная",
        "федерпальное",
        "феднральная",
        "федральное",
    ),
    "NATL": (
        "landelijk",
        "nacional",
        "nacionalna",
        "nacionalnyy",
        "narodowy",
        "nasjonale",
        "national",
        "nationalnyy",
        "nationell",
        "nazionale",
        "národní",
        "ulusal",
        "valstybinis",
        "национальнaя",
        "национальнoго",
        "национальнoе",
        "национальнoй",
        "национальная",
        "национального",
        "национальной",
        "национальные",
        "национальный",
        "национальных",
        "国家的",
        "国立",
    ),
    "REPUBLIC": (
        "cumhuriyet",
        "repubblica",
        "republic",
        "republics",
        "republik",
        "republika",
        "república",
        "respublik",
        "respublika",
        "respublikos",
        "respubliky",
        "république",
        "республик",
        "республика",
        "республиканская",
        "республиканские",
        "республиканский",
        "республиканского",
        "республиканской",
        "республике",
        "республики",
        "جمهورية",
        "共和国",
    ),
    "INSTITUTE": (
        "industite",
        "insitute",
        "institiut",
        "instituição",
        "institut",
        "instituta",
        "institutas",
        "institute",
        "instituti",
        "instituto",
        "institutu",
        "institutul",
        "institutului",
        "instituut",
        "instituutin",
        "instituutti",
        "institūts",
        "instytut",
        "instytutu",
        "inštitut",
        "inštituta",
        "inštitút",
        "istituto",
        "ynstytut",
        "Ινστιτούτο",
        "институт",
        "школа",
        "інститут",
        "学院",
        "研究所",
    ),
    "ACADEMY": (
        "academie",
        "academy",
        "akademi",
        "akademie",
        "akademii",
        "akademiia",
        "akademija",
        "akademiya",
        "akadēmija",
        "akadēmijai",
        "akadēmijas",
        "akadēmijā",
        "akadēmijām",
        "академии",
        "академиии",
        "академиию",
        "академия",
    ),
    "INSTITUTION": (
        "institucija",
        "institution",
        "instituția",
        "учраждение",
        "учреджение",
        "учреждение",
        "учреждениение",
    ),
    "FDN": (
        "Fondacion",
        "Fondacioni",
        "Foundation",
        "Fundación",
        "Stichting",
        "Stiftung",
        "fdn",
        "fondation",
        "foudation",
        "foundation",
        "fundacija",
        "fundacion",
        "fundacja",
        "fundația",
        "fundație",
        "Ίδρυμα",
        "фонд",
        "基金",
        "設立",
        "財団",
    ),
    "PRIVATE": (
        "privat",
        "private",
        "privati",
        "privatiy",
        "privato",
        "приватная",
        "приватное",
        "приватные",
        "приватный",
    ),
    "PUBLIC": (
        "public",
        "publiczna",
        "publiczne",
        "publiczny",
        "общественная",
        "общественного",
        "общественной",
        "общественные",
        "общественный",
        "общественных",
        "公共的",
        "公共的な",
    ),
    "INDUSTRY": (
        "endustriyel",
        "indastri",
        "indastrial",
        "indastris",
        "industra",
        "industria",
        "industriais",
        "industrial",
        "industriala",
        "industriale",
        "industriales",
        "industrialna",
        "industrialny",
        "industrialnyi",
        "industrială",
        "industrias",
        "industrie",
        "industriei",
        "industriel",
        "industriell",
        "industrier",
        "industries",
        "industriia",
        "industriilor",
        "industrija",
        "industrije",
        "industriji",
        "industrijo",
        "industriju",
        "industriya",
        "industriyi",
        "industriële",
        "industrji",
        "industry",
        "indústria",
        "indústrias",
        "индустральный",
        "индустриальный",
        "индустрии",
        "индустрия",
        "промышленности",
        "業界",
    ),
    "ENG": (
        "engineering",
        "engineers",
        "ingegneri",
        "ingenierie",
        "ingeniería",
        "ingeniérie",
        "inzhiniring",
        "inženiring",
        "inženýrství",
        "иинженерно",
        "иинжиниринг",
        "инженерия",
        "инженерный",
        "инжиниринг",
        "инжинириум",
        "エンジニアリング",
        "工学",
    ),
    "SCIENTIFIC": (
        "science",
        "science and technology",
        "sciences",
        "scientific",
        "научная",
        "научно-исследовательский",
        "научного",
        "научное",
        "научной",
        "научные",
        "научный",
        "научных",
    ),
    "HEALTH": ("health", "healthcare", "здравоохранения"),
    "PHARMACY": ("apotheke", "pharmacy", "фармацевтический"),
    "MACHINE": (
        "machine",
        "machinery",
        "machines",
        "machining",
        "машиностроение",
        "машиностроения",
        "машиностроительная",
        "машиностроительного",
        "машиностроительной",
        "машиностроительные",
        "машиностроительный",
        "машиностроительных",
    ),
    "ASSOCIATES": ("associates", "ассоциированные"),
    "NETWORK": ("network", "networks", "netzwerk", "сетевое", "сетевой", "сетевые"),
    "INVESTMENT": (
        "inversiones",
        "inversión",
        "invesment",
        "invesmenti",
        "invest",
        "investasi",
        "investeringen",
        "investice",
        "investicija",
        "investicni",
        "investimenti",
        "investimento",
        "investimentos",
        "investissement",
        "investitorii",
        "investitsiy",
        "investment",
        "investments",
        "investmet",
        "иинвестиционный",
        "инвестиции",
        "инвестиционная",
        "инвестиционного",
        "инвестиционной",
        "инвестиционные",
        "инвестиционный",
        "инвестиционных",
        "інвестиционный",
        "השקעה",
        "投資",
        "투자",
    ),
    "CASINO": (
        "casino",
        "cazinou",
        "kasiino",
        "kasino",
        "kasyno",
        "kaszinó",
        "kasíno",
        "kazino",
        "казино",
    ),
    "LEASING": ("lease", "leasing", "лизинг", "лизинговая", "лизинговые"),
    "DEV": (
        "dev",
        "development",
        "devt",
        "devt.",
        "ανάπτυξη",
        "ανάπτυξης",
        "развитие",
        "развитию",
        "развития",
    ),
    "ENTERPRISE": (
        "empresa",
        "enterpraizes",
        "enterprise",
        "enterprises",
        "entreprise",
        "entreprises",
        "impresa",
        "interprise",
        "predpriyatie",
        "yritys",
        "пpедпpиятие",
        "предприемство",
        "предприиятие",
        "предприятие",
        "предприятий",
        "предприятия",
        "предпрятие",
        "підприємство",
        "प्रतिष्ठान",
        "기업",
    ),
    "EDUCATION": (
        "education",
        "educational",
        "лаборатория",
        "образования",
        "образовательная",
        "образовательное",
        "общеобразовательное",
    ),
    "ISLAMIC": (
        "islam",
        "islami",
        "islamic",
        "islamique",
        "islamiske",
        "islamitische",
        "islamiya",
        "islamiyah",
        "islamo",
        "islamske",
        "islamskiej",
        "islamských",
        "islámských",
        "islâmico",
        "исламская",
        "исламские",
        "исламский",
        "исламских",
        "исламского",
        "исламской",
    ),
    "REINSURANCE": (
        "re",
        "re-assurance",
        "reaseguro",
        "reinsurance",
        "reinsurance company",
        "reinsurer",
        "resseguro",
        "riassicurazione",
        "réassurance",
        "rückversicherung",
        "перестрахование",
        "재보험",
    ),
    "RESEARCH": (
        "forschung",
        "research",
        "изучение",
        "исследование",
        "исследовательский",
    ),
    "RESOURCES": (
        "recurso",
        "recursos",
        "resource",
        "resources",
        "ressource",
        "ressources",
    ),
    "SHIPPING": (
        "schiffahrt",
        "shipping",
        "shipping company",
        "shipping line",
        "shipping lines",
        "судноплавная",
        "судноплавного",
        "судноплавной",
        "судноплавные",
        "судноплавных",
        "судноплавства",
        "судноплавство",
        "судоходная",
        "судоходного",
        "судоходной",
        "судоходные",
        "судоходных",
        "судоходства",
        "судоходство",
        "шиппинг",
    ),
    "AGRICULTURE": (
        "agricola",
        "agricole",
        "agricoltura",
        "agricultura",
        "agricultural",
        "agriculture",
        "agrikultura",
        "агрикультура",
    ),
    "SHIPBUILDING": (
        "shipbuilding",
        "shipyard",
        "shipyards",
        "судостроение",
        "судостроительная",
        "судостроительного",
        "судостроительной",
        "судостроительные",
        "судостроительный",
        "судостроительных",
    ),
    "PRODUCTION": (
        "production",
        "productions",
        "proizvodstvennoe",
        "производственно",
        "производственное",
    ),
    "CONSTRUCTION": (
        "construcao",
        "construcoes",
        "construction",
        "construtora",
        "construção",
        "construções",
        "строительная",
        "строительное",
        "строительный",
        "строительства",
        "строительство",
    ),
    "CONSULTING": (
        "beratung",
        "conseils",
        "constultants",
        "consulenza",
        "consult",
        "consultancy",
        "consultants",
        "consulting",
        "konsult",
        "konsulting",
        "консалтинг",
        "консалтинговая",
        "コンサルティング",
    ),
    "DESIGN": (
        "design",
        "designer",
        "designers",
        "designing",
        "diseñador",
        "diseño",
        "dizayn",
        "progettazione",
        "проектирование",
        "проектировщик",
        "проектировщики",
        "проектировщиц",
        "проектировщица",
        "проектировщицы",
        "デザイン",
    ),
    "REVOLUTIONARY": (
        "revoliucijos",
        "revolucio",
        "revolucionarne",
        "revolucionāro",
        "revolution",
        "revolutionaire",
        "revolutionary",
        "revolutsionny",
        "revolutsioonilise",
        "revolučních",
        "revolučných",
        "rewolucji",
        "rivoluzionarie",
        "révolution",
        "революционная",
        "революционные",
        "революционный",
        "революционных",
    ),
    "COMMITTEE": (
        "committee",
        "committees",
        "комитет",
        "комитета",
        "комитету",
        "комитеты",
        "комітет",
        "комітети",
    ),
    "PROFESSIONAL": (
        "professional",
        "professionalnyi",
        "professionel",
        "профессиональная",
        "профессионально",
        "профессионального",
        "профессиональное",
        "профессиональных",
        "профессиональоного",
        "профессионольного",
        "профессонального",
    ),
    "OPERATIONS": (
        "operaciones",
        "operacyjne",
        "operasional",
        "operasyonel",
        "operating",
        "operation",
        "operationeel",
        "operations",
        "operativa",
        "operativní",
        "operativos",
        "operazioni",
        "operações",
        "opérations",
        "операции",
        "операциях",
        "オペレーション",
        "运营",
    ),
    "KOMBINAT": ("Kombinatas", "kombinat", "комбинат", "комбинированного"),
    "CENTRAL": (
        "central",
        "centrala",
        "centrale",
        "centralele",
        "centrali",
        "централизованная",
        "центральная",
        "центрального",
        "центральной",
        "центральные",
        "центральный",
    ),
    "GENERAL": (
        "general",
        "генеральная",
        "генерального",
        "генеральной",
        "генеральные",
        "генеральный",
        "генеральных",
    ),
    "MECHANICAL": (
        "Mecánica",
        "Mekhanicheskiy",
        "mecanique",
        "mechanical",
        "mechanický",
        "mécanique",
        "МЕХАНИЧЕСКИЙ",
        "Механический",
        "механика",
        "механическая",
        "механические",
        "механический",
        "механических",
        "механического",
        "механической",
    ),
    "TRANSPORT": (
        "transpor",
        "transport",
        "transportation",
        "transporte",
        "trasporto",
        "vervoer",
        "транспорт",
        "транспортировке",
        "транспортировок",
        "транспортная",
        "транспортные",
        "транспортный",
    ),
    "FINANCIAL": (
        "finance",
        "finances",
        "financial",
        "finans",
        "финансирование",
        "финансирования",
        "финансовая",
        "финансового",
        "финансовой",
        "финансовые",
        "финансовый",
        "финансовых",
    ),
    "COMMS": (
        "comms",
        "communication",
        "communications",
        "kommunikation",
        "коммуникации",
        "коммуникаций",
        "коммуникация",
        "коммюникации",
        "связи",
        "связь",
    ),
    "INFORMATION": (
        "info",
        "informatica",
        "informatics",
        "information",
        "informatyka",
        "informatyki",
        "информацией",
        "информационное",
        "информационные",
        "информационных",
        "информация",
        "информациями",
    ),
    "ELECTRONIC": (
        "electronic",
        "electronical",
        "electronics",
        "electrónica",
        "electrónico",
        "elektronisch",
        "elektronische",
        "elektronisk",
        "elettronica",
        "elettronico",
        "électronique",
        "електронна",
        "електронний",
        "електронски",
        "электроника",
        "электроники",
        "электронная",
        "электронные",
        "электронный",
        "электронных",
        "电子",
        "電子",
        "電子の",
        "전자",
        "전자의",
    ),
    "ELECTRIC": (
        "electric",
        "electrical",
        "electrically",
        "electricity",
        "электрическая",
        "электрические",
        "электрический",
        "электрических",
        "электрического",
        "электрической",
    ),
    "ENERGY": (
        "energi",
        "energia",
        "energias",
        "energie",
        "energier",
        "energies",
        "energieën",
        "energija",
        "energije",
        "energií",
        "energy",
        "energía",
        "énergie",
        "énergies",
        "енергија",
        "енергије",
        "енергія",
        "енергії",
        "энергии",
        "энергия",
        "エネルギー",
        "能源",
        "能量",
        "에너지",
    ),
    "POWER": ("power", "powers"),
    "BASE": ("base", "bases", "baza", "база"),
    "CHEMICAL": (
        "chemical",
        "chemicals",
        "химическая",
        "химические",
        "химический",
        "химических",
        "химического",
        "химической",
    ),
    "MUSEUM": (
        "musei",
        "museos",
        "museu",
        "museum",
        "museums",
        "musée",
        "muzej",
        "muzeji",
        "muzeum",
        "музей",
        "музея",
        "музеям",
        "музеями",
        "музеях",
    ),
    "SECURITY": (
        "güvenlik",
        "securities",
        "security",
        "seguridad",
        "sécurité",
        "безопасности",
    ),
    "GLOBAL": (
        "global",
        "globalny",
        "globální",
        "küresel",
        "mondial",
        "mondiale",
        "weltweit",
        "глобальная",
        "глобально",
        "глобального",
        "глобальной",
        "глобальные",
        "глобальный",
        "глобальных",
    ),
    "WORLD": ("welt", "world", "worldwide", "мир", "世界"),
    "SYSTEM": (
        "sistema",
        "sistemas",
        "sistemy",
        "system",
        "systeme",
        "systems",
        "système",
        "systèmes",
        "система",
        "системы",
        "システム",
        "システムズ",
    ),
    "ASSOCIATION": ("association", "associations", "ассоциации", "ассоциация"),
    "BUREAU": ("bureau", "büro", "büron", "büronü", "bürö", "бюрo", "бюро"),
    "VENTURE": (
        "empreendimentos",
        "venture",
        "ventures",
        "венчурная",
        "венчурного",
        "венчурное",
        "венчурной",
        "венчурные",
        "венчурный",
        "венчурных",
    ),
    "LOGISTICS": (
        "logistic",
        "logistics",
        "логистика",
        "логистики",
        "логистическая",
        "логистических",
        "логистической",
        "логистическому",
    ),
    "MARKETING": (
        "markedsføring",
        "marketing",
        "marketinga",
        "marketingo",
        "marketingová",
        "marketingové",
        "marketingových",
        "marketingu",
        "маркетинг",
        "маркетинговая",
        "маркетинговые",
        "маркетинговый",
        "маркетинговых",
    ),
    "COMMERCE": (
        "commerce",
        "commercial",
        "коммерция",
        "коммерческая",
        "коммерческие",
        "коммерческих",
        "коммерческого",
        "коммерческой",
    ),
    "BANK": (
        "Kreditanstalt",
        "banca",
        "banco",
        "bank",
        "banka",
        "bankası",
        "banki",
        "banking",
        "banque",
        "clearing bank",
        "commercial bank",
        "handelsbank",
        "handelsbanken",
        "investment bank",
        "investment banking",
        "merchant bank",
        "savings and loan",
        "savings and loan association",
        "savings bank",
        "trade bank",
        "τράπεζα",
        "банк",
        "банка",
        "банківська",
        "банківська установа",
        "инвестиционный банк",
        "коммерческий банк",
        "муниципально-коммерческий банк",
        "銀行",
    ),
    "COMMERCIAL": (
        "comercial",
        "comerciale",
        "comercială",
        "commercial",
        "commerciale",
        "commerciaux",
        "commercieel",
        "commerciell",
        "commerciële",
        "commerical",
        "kommercial",
        "kommersiel",
        "εμπορική",
        "комерческий",
        "коммерческая",
        "коммерческий",
        "коммерческих",
        "коммерческого",
        "коммерческое",
        "коммерческой",
        "קומרציאלי",
        "商业的",
        "商業",
        "상업적",
    ),
    "OFFSHORE": (
        "offshore",
        "offshores",
        "ofshor",
        "оффшор",
        "оффшорная",
        "оффшорного",
        "оффшорной",
        "оффшорные",
        "оффшорный",
        "オフショア",
    ),
    "MUNICIPAL": (
        "municipal",
        "municipality",
        "munisipalitas",
        "munisipalitetet",
        "munisipaliteti",
        "муниципальная",
        "муниципального",
        "муниципальное",
        "муниципальной",
        "муниципальные",
        "муниципальный",
        "муниципальных",
        "муніципальна",
        "муніципальний",
        "муніципальних",
        "муніципального",
        "муніципальної",
        "муніципальні",
    ),
    "CITY": (
        "cities",
        "city",
        "city council",
        "city government",
        "city of",
        "city of the",
        "municipale",
        "municipale de",
        "municipale des",
        "municipale du",
        "municipalité",
        "municipals",
        "города",
        "городская",
        "городские",
        "городских",
        "городского",
        "муниципалитет",
        "муниципалитета",
        "муниципалитетам",
        "муниципалитеты",
    ),
    "OFFICE": (
        "bureau",
        "bureaux",
        "office",
        "offices",
        "oficina",
        "oficinas",
        "бюро",
        "офис",
        "офиса",
        "офисы",
        "оффис",
        "офіс",
        "офіси",
    ),
    "GOV": (
        "gobierno",
        "gobierno de",
        "gouvernement",
        "gouvernement de",
        "gouvernementale",
        "gov",
        "government",
        "government of",
        "government of the",
        "governmental",
        "govmnt",
        "regierung",
        "regierung der",
        "regierung von",
        "staat",
        "staatlich",
        "staatliche",
        "staatliches",
        "state",
        "state of",
        "κυβερνηση",
        "κυβερνητική",
        "гомударственной",
        "госпредприятие",
        "госудаpственное",
        "государсвенным",
        "государсовенный",
        "государственная",
        "государственне",
        "государственно",
        "государственного",
        "государственное",
        "государственной",
        "государственном",
        "государственноый",
        "государственные",
        "государственный",
        "государственным",
        "государственных",
        "государствнный",
        "госудаственным",
        "госудрарственное",
        "казенное",
        "правительство",
        "правительсто",
        "政府",
        "辖",
    ),
    "REGION": (
        "region",
        "regional",
        "regionala",
        "regionales",
        "regionali",
        "regionalny",
        "regions",
        "област",
        "области",
        "областная",
        "областное",
        "областные",
        "район",
        "района",
        "районов",
    ),
    "NAMED": (
        "im",
        "imeni",
        "named after",
        "named for",
        "named in honor of",
        "named in memory of",
        "им",
        "имени",
        "імені",
    ),
    "SOCALLED": (
        "Așa-numita",
        "Den såkaldte",
        "Javni pokret",
        "L-hekk imsejħa",
        "SO-CALLED",
        "Samhällsrörelsen",
        "Self-proclaimed",
        "Sogenannte",
        "Soi-disant",
        "Takozvana",
        "Tzv.",
        "Tā sauktā",
        "mar a ghairtear de",
        "népi mozgalom",
        "soi-disant",
        "the so-called",
        "Т.нар.",
    ),
    "MINISTRY": (
        "ministerie",
        "ministerij",
        "ministerio",
        "ministerium",
        "ministero",
        "ministerstvo",
        "ministeru",
        "ministerul",
        "ministerului",
        "ministrerio",
        "ministries",
        "ministry",
        "ministère",
        "мinistry",
        "министерства",
        "министерство",
    ),
    "EXPORT": ("eksportu", "export", "exports", "экспорт", "экспорты"),
    "IMPORT": (
        "import",
        "imports",
        "импорт",
        "импорта",
        "импортная",
        "импортного",
        "импортной",
        "импортные",
        "импортный",
        "импортных",
        "импорты",
    ),
    "TRADE": (
        "Dis Ticaret",
        "Diş Ticaret",
        "Foreign Trade",
        "SANAYI VE TICARET",
        "San. ve Tic.",
        "Tesisler Teknoloji Dış",
        "Tesisler Teknoloji Dış Ticaret",
        "Zovnishnotorhovelna",
        "comercio",
        "commercio",
        "comércio",
        "imp. exp.",
        "impex",
        "san. tic.",
        "san.tic",
        "san.ve",
        "sanayi ticaret",
        "sanayi ve ticaret",
        "tes.tek.dis",
        "tes.tek.dış",
        "ticaret",
        "trade",
        "trading",
        "trэid",
        "usaha dagang",
        "Εμπορική",
        "Зовнішньоторговельна",
        "торговая",
        "торговля",
        "торгово",
        "торговы",
        "торговые",
        "торговый",
        "торговых",
        "торговьй",
        "торгівля",
        "商业",
        "商行",
        "行业",
        "贸易",
        "贸易公司",
    ),
    "SOLUTION": (
        "solution",
        "solutions",
        "λύσεις",
        "λύσεων",
        "λύση",
        "решение",
        "решению",
        "решения",
    ),
    "PROJECT": (
        "project",
        "projects",
        "projekat",
        "projekt",
        "projektas",
        "projekti",
        "projektių",
        "projekto",
        "projektoje",
        "projektų",
        "projektųje",
        "projetos",
        "проект",
        "проекта",
        "проектно",
        "проектный",
        "проектов",
        "проекту",
        "проекты",
    ),
    "PRODUCT": (
        "product",
        "products",
        "produkt",
        "produtos",
        "продукция",
        "продукцією",
        "продукціі",
    ),
    "DISTRIB": (
        "distribution",
        "distributor",
        "distributors",
        "дистрибутора",
        "дистрибуции",
        "дистрибуция",
        "дистрибьютор",
    ),
    "CAPITAL": ("capital", "kapital", "κεφάλαιο", "κεφαλαιακή", "капитал"),
    "CREDIT": ("credit", "credits", "kredit", "КРЕДИТНАЯ", "кредитная"),
    "ASSET": ("asset", "assets"),
    "BUDGET": (
        "budget",
        "budgetary",
        "бюджет",
        "бюджетная",
        "бюджетного",
        "бюджетное",
        "бюджетной",
        "бюджетные",
        "бюджетный",
    ),
    "TRUST": (
        "trust",
        "trusts",
        "траст",
        "трастова",
        "трастовий",
        "трастового",
        "трастової",
        "трастові",
        "фонд",
        "фондова",
        "фондовий",
        "фондові",
    ),
    "UNION": (
        "União",
        "savez",
        "szakszervezet",
        "union",
        "unión",
        "zveza",
        "związek",
        "об'єднання",
        "обєднання",
        "союз",
        "союза",
        "ユニオン",
        "組合",
        "联盟",
    ),
    "UNITED": (
        "unidas",
        "unificadas",
        "unificata",
        "unificati",
        "unificato",
        "unifié",
        "unita",
        "united",
        "uniti",
        "об'єднана",
        "об'єднаний",
        "об'єднані",
        "объединенная",
        "объединенные",
        "объединенный",
        "объединенных",
    ),
    "EXCHANGE": ("exchange", "exchanges", "биржа", "биржевое", "биржевой", "биржи"),
    "UNIVERSITY": (
        "uni",
        "univ",
        "univ.",
        "univ. of",
        "univ. of the",
        "universidad",
        "universitet",
        "university",
        "universität",
        "université",
        "universtiy",
        "universytet",
        "университет",
        "университета",
        "университи",
        "университнт",
        "унивирситет",
        "університет",
    ),
    "MILITARY": ("militare", "military", "военный"),
    "TACTICAL": (
        "tactical",
        "tactics",
        "taktik",
        "taktische",
        "taktički",
        "тактическая",
        "тактические",
        "тактический",
        "тактических",
        "тактического",
        "тактической",
        "тактична",
        "тактичного",
        "тактичной",
        "тактичні",
    ),
    "ARMY": (
        "armee",
        "armeen",
        "army",
        "armée",
        "armée de terre",
        "ejército",
        "армейская",
        "армейские",
        "армейский",
        "армейских",
        "армейского",
        "армейской",
        "армия",
        "армія",
    ),
    "MANUFACTURING": (
        "manufacturies",
        "manufacturing",
        "manufakturynh",
        "production",
        "производственная",
        "производственный",
        "производство",
    ),
    "MEDICAL": (
        "medical",
        "medicine",
        "медицинская",
        "медицинские",
        "медицинский",
        "медицинского",
        "медицинской",
        "медична",
        "медичний",
        "медичних",
        "медичні",
    ),
    "HOSPITAL": (
        "hospital",
        "hospitals",
        "больница",
        "больницам",
        "больницах",
        "больницы",
        "госпитале",
        "госпиталь",
        "госпиталя",
        "госпиталям",
        "госпиталями",
        "госпиталях",
    ),
    "SOCIAL": (
        "social",
        "socialnyy",
        "socjalna",
        "socjalne",
        "socjalny",
        "socjalnych",
        "sosyal",
        "социальная",
        "социального",
        "социальной",
        "социальные",
        "социальный",
        "социальных",
    ),
    "MEDIA": ("media", "medios", "médias", "медиа", "медіа"),
    "SCHOOL": (
        "Schule",
        "escola",
        "escuela",
        "school",
        "scuola",
        "skola",
        "szkoła",
        "école",
        "šola",
        "школа",
    ),
    "TPB": (
        "corporația de rachete balistice tactice",
        "društvo za taktičke projektile",
        "tactical missile company",
        "tactical missile corporation",
        "tactical missiles company",
        "tactical missiles corporation",
        "takticheskoe raketnoe vooruzhenie",
        "тактическое ракетное вооружение",
        "тактичне ракетне озброєння",
    ),
    "NPP": (
        "Corporația de cercetare și producție",
        "ENGINEERING AND CONSTRUCTION",
        "Kutató- és Termelővállalat",
        "NAUCHNO-PROIZVODSTVENNOE PREDPRIYATIE",
        "Naukovo-vyrobnyche pidpryiemstvo",
        "Scientific and Production",
        "Scientific and Production Enterprise",
        "npp",
        "research & production",
        "research & production association",
        "research & production corporation",
        "research and production",
        "research and production association",
        "research and production company",
        "research and production corporation",
        "research and production enterprise",
        "Întreprinderea de cercetare și producție",
        "НПП",
        "Науково-виробниче",
        "Науково-виробниче підприємство",
        "Научно-производственное",
        "Научно-производственное предприятие",
    ),
    "LIQUIDATION": (
        "/in liquidation/",
        "en liquidacion",
        "en liquidation",
        "in liqu.",
        "in liquidation",
        "in liquidazione",
        "清算中",
    ),
}

PERSON_SYMBOLS: Dict[str, Tuple[str, ...]] = {
    "JR": ("Jr", "Junior"),
    "SR": ("Senior", "Snr", "Sr"),
    "ESQ": ("Esq", "Esquire"),
    "DR": ("Doctor", "Doktor", "Doktorin", "Dr"),
    "PROF": ("Prof", "Professor"),
    "MD": ("Doctor of Medicine", "MD"),
    "DDS": ("DDS", "Doctor of Dental Surgery"),
    "DVM": ("DVM", "Doctor of Veterinary Medicine"),
    "PHD": ("Doctor of Philosophy", "PhD"),
    "CPA": ("CPA", "Certified Public Accountant"),
    "CFA": ("CFA", "Chartered Financial Analyst"),
    "CMA": ("CMA", "Certified Management Accountant"),
    "CSA": ("CSA", "Certified Senior Advisor", "Certified Senior Associate"),
    "BIN": (
        "ben",
        "bin",
        "bint",
        "ibn",
        "ibn al",
        "ibn ul",
        "ould",
        "בן",
        "בר",
        "ابن",
        "ابن ال",
        "بن",
        "بنت",
    ),
    "BINT": ("bat", "bint", "mint", "ult", "בת", "بنت"),
    "ABU": ("Abdul", "Abu", "Abu al", "Abul", "أبو"),
    "UMM": ("Umm", "Umm al", "Ummul"),
    "BEN": ("Ben", "Benedict", "Benjamin"),
    "BOB": ("Bob", "Rob", "Robert", "Roberto"),
    "DICK": ("Dick", "Richard"),
    "WILL": ("will", "william"),
}
