...
(`maturin develop --release`), LRU caches busted with unique per-iteration inputs.
See `benchmarks/bench_transliteration.py` and `rust/examples/bench_transliterate.rs`.
...
**Actionable optimizations if the regression starts hurting:**
...
- **Swap backend to `anyascii`** (see "Alternative: anyascii" below).
