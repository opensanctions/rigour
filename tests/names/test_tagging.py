from rigour.names import Name, NamePartTag, NameTypeTag, Symbol
...
def test_tag_person_name():
    ...
    # Wikidata QID for "John" (may change if the corpus regenerates).
    ...
    # An Arabic given name — should not crash. With the full Wikidata
    # corpus loaded, whether it matches anything depends on corpus
    # content; just assert the call succeeds and preserves the form.
    name = Name("أسامة")
    tagged_name = tag_person_name(name)
    assert tagged_name is not None
...
def test_tag_person_name_overlapping():
    """Document the overlapping-match behaviour for Korean compound names.

    Each corpus entry maps a Wikidata QID to a form set (e.g. jae-ho →
    jeong-jae overlap); the AC tagger finds every matching phrase and
    the Name's apply_phrase applies each as a separate Span.
    """
    name_ahocor = Name("jeong jae ho")
    name_ahocor = tag_person_name(name_ahocor)
    ...
    all_expected = {jae_ho, jeong, jeong_jae, ho, jae}
    assert all_expected - name_ahocor.symbols == set()
...
def test_tag_org_name_sorting():
    ...
    tagged_name = tag_org_name(name)
    sorted_parts = NamePart.tag_sort(tagged_name.parts)
...
def test_tag_org_spzoo():
    # "Sp. z o.o." — Polish limited company. With Cleanup.Strong + SQUASH_SPACES
    # the alias and haystack both collapse punctuation to whitespace:
    # "sp. z o.o." → "sp z o o". Tagger indexes the display form under
    # that key, so it matches as a multi-token span in the name.
