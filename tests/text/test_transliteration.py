...
These tests pin the current PyICU/normality output so that the Rust/ICU4X port
(rigour-core Phase 1) can detect regressions. Expected values were generated by
running the current implementation — some may look imperfect (e.g. Arabic producing
partial transliterations) but that is the current behavior we need to match or
consciously diverge from.
"""

from normality import ascii_text, latinize_text
...
def test_ascii_norwegian_danish() -> None:
    ...
    assert ascii_text("Lars Løkke Rasmussen") == "Lars Lo/kke Rasmussen"
