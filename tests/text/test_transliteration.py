"""Transliteration tests (ascii_text, latinize_text).

Exercises the Rust/ICU4X path via `rigour.text.transliteration`. Pinned
expected values were originally generated against PyICU/normality; where
ICU4X diverges the pins have been updated with a note explaining the
change. Script detection and predicates live in test_scripts.py.
"""
...
def test_ascii_latin_plus_cyrillic() -> None:
...
def test_ascii_latin_diacritics_plus_han() -> None:
    """French diacritics with Chinese. Diacritic side decomposes via NFKD;
    Han side goes through the Pinyin transliterator."""
...
def test_ascii_three_scripts() -> None:
    out = ascii_text("Hello мир 中国")
    assert out.isascii()
    assert out.lower().startswith("hello ")


def test_ascii_adjacent_scripts_no_whitespace() -> None:
...
def test_ascii_unsupported_script_passthrough() -> None:
...
def test_ascii_pure_punctuation_and_numbers() -> None:
...
# --- Mixed script: latinize_text ---


def test_latinize_preserves_latin_diacritics() -> None:
    assert latinize_text("François Müller") == "François Müller"


def test_latinize_mixed_latin_and_cyrillic() -> None:
...
def test_latinize_adjacent_scripts_no_whitespace() -> None:
    out = latinize_text("Tokyo東京")
    assert "Tokyo" in out
    # No Han letters remain.
    assert not any("\u4E00" <= c <= "\u9FFF" for c in out)


def test_latinize_unsupported_script_passthrough() -> None:
